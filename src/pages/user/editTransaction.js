...
const transactionTypes = [{ 'id': 1, 'name': 'Income' }, { 'id': 2, 'name': 'Expense' },]
...
function EditTransaction() {
    ...
    useEffect(() => {
        console.log(categories, transactionId);
        setFilteredCategories(categories.filter(cat => cat.transaction_type_id === activeTransactionType));
    ...
    const onSubmit = async (data) => {
        ...
        await UserService.update_transaction(
            transactionId, AuthService.getCurrentUser().user.email, data.category, data.description, data.amount, data.date
        ).then(
            (response) => {
                if (response.data.status === "SUCCESS") {
